...
async def _8ball(ctx, *, question:str):
    ...
    if question.lower().startswith('am'):
        answer = ['Ok, I have no opinion on any of your trivial questions. Next!', 'Ok, you definitely show the symptoms, so yes.', 'Ok. you do not show the symptoms, fortunately.', 'Ok, the universe has decided so.', 'Ok... Rest assured, because you are not.', 'Ok... Rest assured, because you are.', 'Ok... Regardless of what you feel you are, just know that you are loved.', 'Ok, I raise you a better question... Are you stupid?']
    ...
    embed = discord.Embed(title=f'🔮 What is troubling you?', description=f'{Emoji.hamiltonConfuse} You ask the question, \"**{question.capitalize()}?**\"\n\n{Emoji.hamiltonWisdom} and I answer... \"**{random.choice(answer)}**\"', color=0x55ffff)
    await ctx.send(embed=embed)
